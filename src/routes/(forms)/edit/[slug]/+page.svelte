<script lang="ts">
	...
	let cTitleError: string = 'invisible';
	let cTitleMsg: string = 'bg-primary-700 border-primary-700';
	let cSubmit: string = 'bg-primary-700 border-primary-700';
	...
	const callGenerate = async () => {
		if (item) {
			...
			item.password = await generatePassword($SettingStore);
	...
	<div class="flex justify-center items-center h-full overflow-y-scroll">
		<div class="w-full drop-shadow-xl xl:mt-20 h-[70%] rounded-md">
			<div class="flex items-center justify-center">
				<h1 class="flex text-3xl font-bold">Edit {item.title}</h1>
			</div>
			<form
				class="flex flex-col text-lg w-full h-full items-center"
				on:submit|preventDefault={editItem}
			>
				<div class="flex flex-col mt-10 w-[60%]">
					...
					/>
					<span class={`${cTitleError} mt-2`}>Title cannot be empty!</span>
					{#if item.type_ == 'password'}
						<input
				...
				<div class="flex mt-10 mb-10 justify-center">
					...
					<button
						type="submit"
						class={`${cSubmit} hover:bg-primary-700 border duration-200 border-primary-600 rounded-md p-2 flex justify-center items-center bg-primary-500`}
						>Update</button
					>
				</div>
			</form>
